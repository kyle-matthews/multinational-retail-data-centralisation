...
SELECT dim_store_details.store_type, dim_products.product_price,
orders_table.product_quantity
FROM orders_table
INNER JOIN dim_store_details ON orders_table.store_code =
dim_store_details.store_code
INNER JOIN dim_products ON orders_table.product_code =
dim_products.product_code
),
cte2 AS(
SELECT store_type, ROUND(SUM(product_price * product_quantity)) AS
total_sales
FROM product_details
GROUP BY store_type
)
SELECT store_type, total_sales, ROUND(total_sales * 100 / (SELECT
SUM(total_sales) FROM cte2)) AS percentage_total
FROM cte2
